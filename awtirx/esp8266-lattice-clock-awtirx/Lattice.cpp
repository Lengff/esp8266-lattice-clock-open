...
void Lattice::init()
{
  LEDS.addLeds<LED_TYPE, WSPIN, COLOR_ORDER>(leds, 256);          // 初始化光带
...
void Lattice::setBrightness(uint8_t bright, bool save)
{
  ...
  FastLED.setBrightness((bright + 1) * 10); // 设置光带亮度
}
...
void Lattice::refreshLed()
{
  if (!latticeSetting.direction)
  {
    ...
    for (int k = 0; k < columnLength; k++)
    {
      for (int i = 0; i < rowLength; i++)
      {
        for (int j = 0; j < 8; j++)
        {
          int index = (j + (j % 2)) * 8 - (j % 2) + (j % 2 == 0 ? i : (-1 * i));
          leds[index + k * 64] = data[columnLength - k - 1][i] & (0x80 >> j) ? showColor : CRGB::Black;
        }
      }
    }
    FastLED.show();
  ...
  else
  {
    ...
    for (int k = 0; k < columnLength; k++)
    {
      for (int i = 0; i < rowLength; i++)
      {
        uint8_t dt = data[columnLength - k - 1][rowLength - i - 1];
        for (int j = 0; j < 8; j++)
        {
          int index = (j + (j % 2)) * 8 - (j % 2) + (j % 2 == 0 ? i : (-1 * i));
          leds[index + k * 64] = dt & (0x80 >> j) ? showColor : CRGB::Black;
        }
      }
    }
    FastLED.show();
...
void Lattice::upOrDownMove(uint8_t index, uint8_t region, bool direction)
{
  if (direction)
  {
    ...
    {
      data[index][k] =
          (data[index][k] & (region ^ 0xff)) + (data[index][k + 1] & region);
    }
...
void Lattice::showTime3(uint8_t *arr, void (*callback)())
{
  ...
  for (int i = 0; i < 16; i++)
  {
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      uint8_t geweibuff = number_font_animations[gw][i],
              shiweibuff = number_font_animations[sw][i];
      if (geweibuff != 0x0)
      {
...
void Lattice::showCountDownTime(long remain, uint8_t *arr, bool showmode,
                                bool minutechange)
{
  uint8_t mfs[4] = {0x0, 0x0, 0x0, 0x0};
  uint8_t d, h, m;
